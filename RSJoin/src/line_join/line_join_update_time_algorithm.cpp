...
LineJoinUpdateTimeAlgorithm::~LineJoinUpdateTimeAlgorithm() {
...
void LineJoinUpdateTimeAlgorithm::init(int length, const std::string &file) {
    this->n = length;
    this->input = file;
...
void LineJoinUpdateTimeAlgorithm::run(bool print_info) {
    ...
    while (getline(in, line), !line.empty()) {
        if (line.at(0) == '-') {
            ...
            auto tuple_start_time = std::chrono::system_clock::now();
            process(line);
            auto tuple_end_time = std::chrono::system_clock::now();
...
void LineJoinUpdateTimeAlgorithm::process(const std::string &line) {
