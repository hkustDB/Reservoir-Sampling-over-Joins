...
#include "../../include/qz/qz_fk_group_right_batch.h"
#include "../../include/qz/qz_fk_middle_batch.h"
#include "../../include/qz/qz_rows.h"

QzFkGroupAlgorithm::~QzFkGroupAlgorithm() {
    delete reservoir;
    delete left_batch;
    delete middle_batch;
    delete right_batch;
    ...
    for (auto &iter : *category_id_to_group_middle_row) {
        auto vec = iter.second;
        vec->clear();
        delete vec;
    }
    category_id_to_group_middle_row->clear();
    delete category_id_to_group_middle_row;

    for (auto &iter : *income_band_sk_to_group_middle_row) {
        auto vec = iter.second;
        vec->clear();
        delete vec;
    }
    income_band_sk_to_group_middle_row->clear();
    delete income_band_sk_to_group_middle_row;
    ...
    for (auto &p : *category_id_with_income_band_sk_to_group_middle_row) {
        auto grouped_middle_row = p.second;
        for (auto &row : grouped_middle_row->group) {
            delete row->store_sales_row;
            delete row;
        }
        delete grouped_middle_row;
    }
    delete category_id_with_income_band_sk_to_group_middle_row;

    delete qz_fk_group_attribute_item_middle;
    delete qz_fk_group_attribute_middle_right;
}

void QzFkGroupAlgorithm::init(const std::string &file, int k) {
    this->input = file;

    this->reservoir = new Reservoir<QzRow>(k);

    this->category_id_to_item_row = new std::unordered_map<int, std::vector<ItemRow*>*>;
    this->category_id_to_group_middle_row = new std::unordered_map<int, std::vector<QzFkGroupMiddleRow*>*>;
    this->income_band_sk_to_group_middle_row = new std::unordered_map<int, std::vector<QzFkGroupMiddleRow*>*>;
    ...
    this->category_id_with_income_band_sk_to_group_middle_row = new std::unordered_map<unsigned long, QzFkGroupMiddleRow*>;

    this->qz_fk_group_attribute_item_middle = new QzFkGroupAttributeItemMiddle;
    this->qz_fk_group_attribute_middle_right = new QzFkGroupAttributeMiddleRight;

    qz_fk_group_attribute_item_middle->set_left_relation_index(category_id_to_item_row);
    qz_fk_group_attribute_item_middle->set_right_relation_index(category_id_to_group_middle_row);
    qz_fk_group_attribute_item_middle->set_right_attribute(qz_fk_group_attribute_middle_right);

    qz_fk_group_attribute_middle_right->set_left_relation_index(income_band_sk_to_group_middle_row);
    qz_fk_group_attribute_middle_right->set_right_relation_index(income_band_sk_to_right_row);
    qz_fk_group_attribute_middle_right->set_left_attribute(qz_fk_group_attribute_item_middle);

    this->left_batch = new QzFkGroupLeftBatch(qz_fk_group_attribute_item_middle, qz_fk_group_attribute_middle_right);
    this->middle_batch = new QzFkMiddleBatch;
    this->right_batch = new QzFkGroupRightBatch(qz_fk_group_attribute_item_middle, qz_fk_group_attribute_middle_right);
}

Reservoir<QzRow>* QzFkGroupAlgorithm::run(bool print_info) {
    ...
    if (print_info) {
        unsigned long propagation_cnt = qz_fk_group_attribute_item_middle->get_propagation_cnt()
                                        + qz_fk_group_attribute_middle_right->get_propagation_cnt();
...
void QzFkGroupAlgorithm::process(const std::string &line) {
    ...
    switch (sid) {
            ...
            if (ss_customer_sk != 0) {
                ...
                if (iter1 != customer_sk_to_customer_row->end()) {
                    ...
                    if (iter2 != item_sk_to_item_row2->end()) {
                        ...
                        unsigned long category_id_with_income_band_sk = (static_cast<unsigned long>(item_row2->i_category_id) << 32)
                                                                    + household_demographics_row->hd_income_band_sk;
                        auto iter3 = category_id_with_income_band_sk_to_group_middle_row->find(category_id_with_income_band_sk);
                        if (iter3 != category_id_with_income_band_sk_to_group_middle_row->end()) {
                            auto grouped_middle_row = iter3->second;
                            grouped_middle_row->group.emplace_back(middle_row);

                            unsigned long wlcnt = qz_fk_group_attribute_item_middle->get_wlcnt(item_row2->i_category_id);
                            unsigned long wrcnt = qz_fk_group_attribute_middle_right->get_wrcnt(household_demographics_row->hd_income_band_sk);

                            if (grouped_middle_row->group.size() > grouped_middle_row->wcnt) {
                                unsigned long old_wcnt = grouped_middle_row->wcnt;
                                grouped_middle_row->wcnt = grouped_middle_row->wcnt * 2;
                                unsigned long new_wcnt = grouped_middle_row->wcnt;

                                if (wlcnt > 0) {
                                    qz_fk_group_attribute_middle_right->update_from_left(grouped_middle_row, wlcnt * old_wcnt, wlcnt * new_wcnt);
                                }

                                if (wrcnt > 0) {
                                    qz_fk_group_attribute_item_middle->update_from_right(grouped_middle_row, wrcnt * old_wcnt, wrcnt * new_wcnt);
                                }
                            }

                            if (wlcnt > 0 && wrcnt > 0) {
                                middle_batch->init(qz_fk_group_attribute_item_middle->get_left_layer(item_row2->i_category_id),
                                                   middle_row,
                                                   qz_fk_group_attribute_middle_right->get_right_layer(household_demographics_row->hd_income_band_sk));
                                middle_batch->fill(reservoir);
                            }
                        } else {
                            auto grouped_middle_row = new QzFkGroupMiddleRow(item_row2->i_category_id, household_demographics_row->hd_income_band_sk);
                            grouped_middle_row->group.emplace_back(middle_row);
                            grouped_middle_row->wcnt = 1;
                            category_id_with_income_band_sk_to_group_middle_row->emplace(category_id_with_income_band_sk, grouped_middle_row);

                            auto iter4 = category_id_to_group_middle_row->find(item_row2->i_category_id);
                            if (iter4 != category_id_to_group_middle_row->end()) {
                                iter4->second->emplace_back(grouped_middle_row);
                            } else {
                                category_id_to_group_middle_row->emplace(item_row2->i_category_id,
                                                                   new std::vector<QzFkGroupMiddleRow *>{grouped_middle_row});
                            }

                            auto iter5 = income_band_sk_to_group_middle_row->find(household_demographics_row->hd_income_band_sk);
                            if (iter5 != income_band_sk_to_group_middle_row->end()) {
                                iter5->second->emplace_back(grouped_middle_row);
                            } else {
                                income_band_sk_to_group_middle_row->emplace(household_demographics_row->hd_income_band_sk,
                                                                      new std::vector<QzFkGroupMiddleRow *>{grouped_middle_row});
                            }

                            unsigned long wlcnt = qz_fk_group_attribute_item_middle->get_wlcnt(item_row2->i_category_id);
                            if (wlcnt > 0) {
                                qz_fk_group_attribute_middle_right->update_from_left(grouped_middle_row, 0, wlcnt);
                            }

                            unsigned long wrcnt = qz_fk_group_attribute_middle_right->get_wrcnt(household_demographics_row->hd_income_band_sk);
                            if (wrcnt > 0) {
                                qz_fk_group_attribute_item_middle->update_from_right(grouped_middle_row, 0, wrcnt);
                            }
        ...
        case 1: {
            ...
            if (c_current_hdemo_sk != 0) {
                ...
                if (iter1 != hdemo_sk_to_household_demographics_row2->end()) {
                    ...
                    qz_fk_group_attribute_middle_right->update_from_right(right_row);
        ...
        case 2: {
            ...
            qz_fk_group_attribute_item_middle->update_from_left(item_row);
