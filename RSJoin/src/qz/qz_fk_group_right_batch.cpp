...
QzFkGroupRightBatch::QzFkGroupRightBatch(QzFkGroupAttributeItemMiddle *qz_fk_group_attribute_item_middle,
...
void QzFkGroupRightBatch::clear() {
    position = 0;
...
void QzFkGroupRightBatch::set_right_row(QzFkRightRow *row) {
    clear();
    right_row = row;
    size = qz_fk_group_attribute_middle_right->get_lcnt(row->household_demographics_row2->hd_income_band_sk);
    if (size > 0) {
        middle_layer = qz_fk_group_attribute_middle_right->get_left_layer(row->household_demographics_row2->hd_income_band_sk);
    }
}

void QzFkGroupRightBatch::fill(Reservoir<QzRow> *reservoir) {
    if (size > 0) {
...
void QzFkGroupRightBatch::init_real_iterator() {
    ...
    item_layer = qz_fk_group_attribute_item_middle->get_left_layer(middle_row->item_row2->i_category_id);
    item_layer_index = 0;
...
bool QzFkGroupRightBatch::has_next_real() {
    return next_row != nullptr;
}

QzRow* QzFkGroupRightBatch::next_real() {
...
void QzFkGroupRightBatch::compute_next_real() {
    ...
    if (item_layer_index < item_layer->size() - 1) {
        ...
        position_after_next_real += (qz_fk_group_attribute_item_middle->get_wlcnt(middle_row->item_row2->i_category_id)
                                     - qz_fk_group_attribute_item_middle->get_lcnt(middle_row->item_row2->i_category_id));
    }
    ...
    if (middle_group_index < group_middle_row->group.size() - 1) {
        ...
        position_after_next_real += (group_middle_row->wcnt - group_middle_row->group.size())
                                    * (qz_fk_group_attribute_item_middle->get_wlcnt(middle_row->item_row2->i_category_id));
    }
    ...
    if (middle_layer_index < middle_layer->at(middle_layer_unit_size)->size() - 1) {
        ...
        next_row = new QzRow(item_row,
                             ...
                             right_row->household_demographics_row2, right_row->customer_row2);
        return;
    } else if (middle_layer_unit_size < qz_fk_group_attribute_middle_right->get_left_layer_max_unit_size(right_row->household_demographics_row2->hd_income_band_sk)) {
        middle_layer_unit_size *= 2;
        while (middle_layer->find(middle_layer_unit_size) == middle_layer->end()) {
            middle_layer_unit_size *= 2;
        }
...
void QzFkGroupRightBatch::fill_with_skip(Reservoir<QzRow> *reservoir) {
    LOOP:while (true) {
        if (size - position <= reservoir->get_t()) {
            ...
            middle_layer_unit_size = 1;
            while (middle_layer_unit_size <= qz_fk_group_attribute_middle_right->get_left_layer_max_unit_size(right_row->household_demographics_row2->hd_income_band_sk)) {
            ...
            group_middle_row = middle_layer->at(middle_layer_unit_size)->at(middle_layer_index);
            unsigned long wlcnt = qz_fk_group_attribute_item_middle->get_wlcnt(group_middle_row->i_category_id);
            middle_group_index = offset / wlcnt;
            ...
            offset = offset - middle_group_index * wlcnt;
            middle_row = group_middle_row->group.at(middle_group_index);

            if (offset >= qz_fk_group_attribute_item_middle->get_lcnt(middle_row->item_row2->i_category_id)) {
                reservoir->update_t();
                // we get a dummy tuple, update reservoir.t and try next pick
                goto LOOP;
            }
