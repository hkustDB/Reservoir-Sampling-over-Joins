...
QzAttributeItemStoreSales::~QzAttributeItemStoreSales() {
...
void QzAttributeItemStoreSales::set_left_attribute(QzAttributeItemItem *attribute) {
    this->left_attribute = attribute;
}

void QzAttributeItemStoreSales::set_right_attribute(QzAttributeStoreSalesCustomer *attribute) {
    this->right_attribute = attribute;
}

void QzAttributeItemStoreSales::set_left_relation_index(std::unordered_map<int, std::vector<ItemRow*>*>* relation_index) {
    this->left_relation_index = relation_index;
}

void QzAttributeItemStoreSales::set_right_relation_index(std::unordered_map<int, std::vector<StoreSalesRow*>*>* relation_index) {
    this->right_relation_index = relation_index;
}

void QzAttributeItemStoreSales::update_from_left(ItemRow* row, unsigned long old_count, unsigned long new_count) {
    propagation_cnt += 1;
    int key = row->i_item_sk;
...
void QzAttributeItemStoreSales::update_from_right(StoreSalesRow* row, unsigned long old_count, unsigned long new_count) {
    propagation_cnt += 1;
    int key = row->ss_item_sk;
...
unsigned long QzAttributeItemStoreSales::get_lcnt(int key) {
...
unsigned long QzAttributeItemStoreSales::get_wlcnt(int key) {
...
unsigned long QzAttributeItemStoreSales::get_rcnt(int key) {
...
unsigned long QzAttributeItemStoreSales::get_wrcnt(int key) {
...
std::unordered_map<unsigned long, std::vector<ItemRow*>*>* QzAttributeItemStoreSales::get_left_layer(int key) {
    return left_layers.at(key);
}

std::unordered_map<unsigned long, std::vector<StoreSalesRow*>*>* QzAttributeItemStoreSales::get_right_layer(int key) {
    return right_layers.at(key);
}

unsigned long QzAttributeItemStoreSales::get_left_layer_max_unit_size(int key) {
    return left_layer_max_unit_sizes.at(key);
}

unsigned long QzAttributeItemStoreSales::get_right_layer_max_unit_size(int key) {
    return right_layer_max_unit_sizes.at(key);
}

void QzAttributeItemStoreSales::propagate_to_right(int key, unsigned long old_count, unsigned long new_count) {
...
void QzAttributeItemStoreSales::propagate_to_left(int key, unsigned long old_count, unsigned long new_count) {
...
void QzAttributeItemStoreSales::update_left_layer(int key, ItemRow* row, unsigned long old_count, unsigned long new_count) {
    auto iter = left_layers.find(key);
    if (iter != left_layers.end()) {
        std::unordered_map<unsigned long, std::vector<ItemRow*>*>* map = iter->second;
        if (old_count > 0) {
            // remove in previous layer
            std::vector<ItemRow*>* old_layer = map->at(old_count);
            if (old_layer->size() == 1) {
                map->erase(old_count);
            } else {
                // swap with the last element
                unsigned long last_index = old_layer->size() - 1;
                ItemRow* lastElement = old_layer->at(last_index);
        ...
        if (iter2 != map->end()) {
            ...
            auto* layer = new std::vector<ItemRow*>{row};
        ...
        auto* layer = new std::vector<ItemRow*>{row};
        left_layers.emplace(key, new std::unordered_map<unsigned long, std::vector<ItemRow*>*> {{new_count, layer}});
        left_layer_indices[*row] = 0;
        left_layer_max_unit_sizes.emplace(key, new_count);
    }
}

void QzAttributeItemStoreSales::update_right_layer(int key, StoreSalesRow* row, unsigned long old_count, unsigned long new_count) {
...
unsigned long QzAttributeItemStoreSales::get_propagation_cnt() {
